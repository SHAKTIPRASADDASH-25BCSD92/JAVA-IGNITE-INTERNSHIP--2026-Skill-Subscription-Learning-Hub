...
public class SkillPackController {
    ...
    // ─────────────────────────────────────────────────────────
    // GET /packs — Show all skill packs
    // ─────────────────────────────────────────────────────────
    @GetMapping("/packs")
    public String viewPacks(Model model) {
        // STEP 1: Fetch all packs from the service layer
        List<SkillPack> list = packService.getAllPacks();
 
        // STEP 2: Bind the list to the model so packs.jsp can access it
        //         In JSP: <c:forEach var="pack" items="${packs}">
        model.addAttribute("packs", list);
 
        // STEP 3: Return the logical view name → resolves to /WEB-INF/views/packs.jsp
        return "packs";
    }
    ...
    public String addPack(@ModelAttribute SkillPack pack) {
        ...
        //         @ModelAttribute automatically maps form fields → SkillPack fields
        //         e.g. form field name="title" → pack.setTitle(...)
        packService.addSkillPack(pack);
 
        // STEP 2: Redirect to /packs after saving
        //         "redirect:" tells Spring to send HTTP 302 — browser makes a fresh GET /packs
        //         This prevents form resubmission if the user hits browser refresh (POST→Redirect→GET)
